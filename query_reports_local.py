...
def main():
    parser = argparse.ArgumentParser(description="Query and search reports stored in ChromaDB using local embedding models")
    
    # Add local embedding model options
    parser.add_argument("--db-dir", default="./local_chroma_db",
                        help="Directory to store ChromaDB data (default: ./local_chroma_db)")
    parser.add_argument("--model-cache", default="./model_cache",
                        help="Directory to cache the embedding model (default: ./model_cache)")
    ...
    # Initialize LocalReportStore with specified parameters
    print(f"Using local embedding model: {args.model_name}")
    print(f"Model cache directory: {args.model_cache}")
    print(f"ChromaDB directory: {args.db_dir}")
    ...
    try:
        ...
        # Initialize the store with local embedding model
        store = LocalReportStore(
            persist_directory=args.db_dir,
            model_cache_dir=args.model_cache
        )
        
        # If a different model name is specified, update the embedding function
        if args.model_name != "sentence-transformers/all-MiniLM-L6-v2":
        ...
        print("Error: sentence-transformers not installed. Please install it with:")
        print("pip install sentence-transformers")
        sys.exit(1)
    except Exception as e:
        print(f"Error initializing local embedding model: {str(e)}")
        sys.exit(1)
